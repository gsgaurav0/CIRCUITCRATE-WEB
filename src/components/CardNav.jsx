...
const CardNav = ({
    ...
    const toggleMenu = () => {
        const tl = tlRef.current;
        if (!tl) {
            // Recreate timeline if it doesn't exist
            const newTl = createTimeline();
            if (newTl) {
                tlRef.current = newTl;
                if (!isExpanded) {
        ...
        if (!isExpanded) {
            setIsHamburgerOpen(true);
            setIsExpanded(true);
            // Ensure timeline is at the start
            tl.progress(0);
            tl.play(0);
        } else {
            setIsHamburgerOpen(false);
            // Clear any existing callbacks to avoid conflicts
            ...
            >
                <div className="card-nav-top">
                    ...
                    <div className="logo-container">
                        <img src={logo} alt={logoAlt} className="cn-logo" />
                    </div>
                ...
                <div
                    className={`card-nav-content ${isExpanded ? 'visible' : 'invisible'}`}
